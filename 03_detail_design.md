...
本システムは、ユーザー認証およびユーザー管理機能を備えたWebアプリケーションである。
一般ユーザーは新規登録・ログイン・自身のユーザー情報の閲覧および編集を行うことができ、管理者はユーザー一覧の閲覧、ユーザー情報の編集および削除を行うことができる。

## 1.2 対象範囲

本詳細設計書では、以下の機能を対象とする。
...
|:---|:---|:---|:---|:---|
|GET|/users/sign_in|Devise::SessionsController|new|ログイン画面表示|
|POST|/users/sign_in|Devise::SessionsController|create|ログインを実行|
|DELETE|/users/sign_out|Devise::SessionsController|destroy|ログアウトを実行|
|GET|/users/sign_up|Devise::RegistrationsController|new|新規登録画面を表示|
|POST|/users|Devise::RegistrationsController|create|ユーザー登録を実行|
...
|項目|必須|その他|エラー時の動作|
|:--|:--|:--|:--|
|メールアドレス|○|メールアドレス形式|メッセージ表示<br>「メールアドレスまたはパスワードが間違っています」
|パスワード|○|-|メッセージ表示<br>「メールアドレスまたはパスワードが間違っています」
...
sequenceDiagram
    ...
    Admin->>Browser:削除ボタンをクリック
    Browser-->>Admin:確認ダイアログを表示
    Admin->>Browser:確認ボタンをクリック
    Browser->>Rails:ユーザー削除リクエスト
    Rails->>Rails:管理者権限を確認
    Rails->>DB:指定ユーザーを削除
    DB-->>Rails:削除結果返却
    Rails-->>Browser:ユーザー一覧画面へリダイレクト
    Browser-->>Admin:ユーザー一覧画面を表示
